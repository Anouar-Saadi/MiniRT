...
/*   Updated: 2020/12/02 17:19:40 by asaadi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double			inters_ray_cylinder(t_data *data, t_objects *obj, t_ray *ray)
{
	...
	k[0] = 1 - pow(vec_dot(ray->direction, obj->cylinder.normal), 2);
	k[1] = 2 * (vec_dot(ray->direction, oc) - (vec_dot(ray->direction,
					obj->cylinder.normal) * vec_dot(oc, obj->cylinder.normal)));
	k[2] = vec_dot(oc, oc) - pow(vec_dot(oc, obj->cylinder.normal), 2) -
	...
	m[0] = vec_dot(ray->direction, obj->cylinder.normal) *
		t[0] + vec_dot(oc, obj->cylinder.normal);
	m[1] = vec_dot(ray->direction, obj->cylinder.normal) *
		t[1] + vec_dot(oc, obj->cylinder.normal);
